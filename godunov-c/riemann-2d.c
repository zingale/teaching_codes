...
int riemann_(int idir, double *gamma_pass,
	     double *dens_l_pass, double *xmom_l_pass, 
	     double *ymom_l_pass, double *ener_l_pass, 
	     double *dens_r_pass, double *xmom_r_pass, 
	     double *ymom_r_pass, double *ener_r_pass, 
	     double *F_dens_pass, double *F_xmom_pass, 
	     double *F_ymom_pass, double *F_ener_pass) {


  double gamma, dens_l, xmom_l, ymom_l, ener_l, dens_r, xmom_r, ymom_r, ener_r;
  ...
  double vlft, ulft, utlft, plft, clft;
  double vrght, urght, utrght, prght, crght;
  ...
  if (idir == 1) {
    ...
    urght = ymom_r/dens_r;
    utrght = xmom_r/dens_r;
  }

  plft = (ener_l - 0.5*(xmom_l*xmom_l + ymom_l*ymom_l)/dens_l)*(gamma - 1.0);
  plft = max(smallp, plft);

  prght = (ener_r - 0.5*(xmom_r*xmom_r + ymom_r*ymom_r)/dens_r)*(gamma - 1.0);
  ...
  */
  ...
  uts = utlft*scratch + utrght*scratch2;
  ...
  if (idir == 1) {
    ...
    *F_ymom_pass = rhoav*utav*uav;
    *F_ener_pass = uav*(pav/(gamma - 1.0) + 0.5*rhoav*(uav*uav + utav*utav) + pav);

  } else if (idir == 2) {
    *F_dens_pass = rhoav*uav;
    *F_xmom_pass = rhoav*utav*uav;
    *F_ymom_pass = rhoav*uav*uav + pav;
    *F_ener_pass = uav*(pav/(gamma - 1.0) + 0.5*rhoav*(uav*uav + utav*utav) + pav);
  }
