...
! u_t + a u_x = 0
!
! in this code, we assume that a > 0
!
! M. Zingale
...
program upwind
  ...
  ! specify the advective velocity
  ...
  ! compute the number of steps needed to advect 5 times
  nsteps = 5*(xmax-xmin)/(cfl*dx) + 1
  ...
  print *, '# advection problem: u_t + a u_x = 0'
  print *, '# a = ', a
  print *, '# init = ', init
  print *, '# islopetype = ', islopetype
  print *, '# cfl = ', cfl, ' nsteps = ', nsteps
...
subroutine states(nx, ng, dx, dt, a, islopetype, u, ul, ur)
  ...
  do i = imin-1, imax+1
     ...
     if (islopetype == 1) then
        ...
        ! centered difference (equivalent to Lax-Wendroff)
...
subroutine riemann(nx, ng, a, ul, ur, f)
  ...
  ! since we are doing the linear advection eq, and the advective velocity
  ! is always positive, the solution is just the left state
  do i = imin, imax+1
     f(i) = a*ul(i)
  enddo
