! Full Multigrid Example
!
! Do full multigrid for the problem  u'' = g(x), u(0) = 0, u(1) = 1
! with g(x) = cos(x)
!
...
! Outline:
!
! Full multigrid begins with the solution on the coarse mesh and prolongs
! that to the finer mesh, providing a good initial guess of the solution
! on the fine mesh.  Here, a V-cycle is performed to refine the solution,
! and it is then prolonged to the next finest mesh, followed by a V-cycle,
! etc.  By the time we get up to the finest mesh, we have a good initial
! guess at the solution, and we can perform successive V-cycles on the
! finest mesh to further reduce the error.
!
! grid
!
! 5                                 x               x               x
...
!
!
!   |<-------------- full multigrid --------------->|<- subsequent V-cycles ->
!
! In reality, we do not know the true solution to the problem at hand,
! so we compute the error as the ratio of the residual to source term
! norm.  The norm of the true error and the residual error is output
! in multigrid.out for each V-cycle.
!
...
program fmgfv
  ...
  integer, parameter :: nx = 32            ! number of interior zones
  integer, parameter :: ng = 1              ! number of guardcells

  integer, parameter :: nsmooth = 3         ! number of smoothing iterations
  integer, parameter :: num_v_cycles = 10   ! number of subsequent V-cycles
  ...
  write (10,*) '# cycle,   true error,   residual error'
  ...
  do j = 1, num_v_cycles
     ...
     temp2 = error(nx, ng, dx, w)/source_norm
     print *, 'ratio of residual error to source norm = ', &
          temp2

     write (10,*) j, temp, temp2
  enddo
...
!  do i = imin, imax
!     temp = (i - ng - 1 + 0.5d0)*dx + xmin
!     print 100, temp, v(i,nlevels), true(temp), v(i,nlevels) - true(temp)
!  enddo
  ...
  do m = 1, nsmooth
     ...
     do color = 0, 1
        do i = imin+ioff, imax, 2
           v(i) = theta*v(i) + (1.d0 - theta)*0.5d0*(v(i-1) + v(i+1) - dx*dx*f(i))
        enddo
