...
program fmgfv
  ...
  integer, parameter :: nsmooth = 500000   ! max number of smoothing blocks

  double precision, parameter :: tol = 1.d-12 ! tolerance for relative
                                              ! solution error
  ...
  double precision :: true_error, solution_error
  ...
  write (10,*) '# cycle,   true error,   solution error'


  solution_error = 1.d33

  j = 1
  do while (j < nsmooth .and. solution_error > tol)
     ...
     if (mod(j, 100) == 0) then
        write (10,*) j, true_error, solution_error
     endif
  ...
  print *, 'after ', j, ' iterations, L2 norm of the error is ', true_error
